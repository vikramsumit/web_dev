...
export default function Page() {
  ...
  const emailActive = email.length > 0 || emailFocused;
  const passwordActive = password.length > 0 || passwordFocused;
    ...
    <>
      ...
      <main className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 to-slate-800 p-8 font-sans">
        <section className="w-full max-w-md">
          <div
            className={`relative rounded-2xl border bg-white/3 border-white/6 p-7 shadow-2xl text-slate-100 backdrop-blur-md transform transition-all duration-150 ${submitted ? "scale-99 ring-2 ring-emerald-300/20" : ""
              } ${shaking ? "animate-shake border-rose-400/25" : ""} animate-floatIn`}
            role="region"
            aria-label="Login form"
          >
            <form id="login-form" onSubmit={handleSubmit} noValidate className="grid gap-3">
              <h2 className="text-white text-lg font-semibold justify-center items-center">Login Here Bro</h2>
              ...
              <div className="relative flex items-center gap-3 bg-white/2 rounded-xl p-3 border border-white/5 focus-within:border-indigo-500/90 focus-within:shadow-md transition">
                <span className="text-slate-400 flex-none" aria-hidden>
                  <svg width="18" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                ...
                <div className="flex-1 relative">
                  <input
                    ...
                    aria-required="true"
                    autoComplete="email"
                    className="w-full bg-transparent border-none text-slate-100 placeholder-transparent outline-none text-sm pt-4 pb-1"
                    placeholder="Email"
                  />
                  <label
                    htmlFor="email"
                    className={`absolute left-10 transition-all pointer-events-none ${emailActive ? "-translate-y-3 scale-90 top-1 text-xs text-slate-300" : "top-3 text-sm text-slate-400"
                      }`}
                  >
                    Email
                  </label>
              ...
              <div className="relative flex items-center gap-3 bg-white/2 rounded-xl p-3 border border-white/5 focus-within:border-indigo-500/90 focus-within:shadow-md transition">
                <span className="text-slate-400 flex-none" aria-hidden>
                  <svg width="16" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                ...
                <div className="flex-1 relative">
                  ...
                  <label
                    htmlFor="password"
                    className={`absolute left-10 transition-all pointer-events-none ${passwordActive ? "-translate-y-3 scale-90 top-1 text-xs text-slate-300" : "top-3 text-sm text-slate-400"
                      }`}
                  >
                    Password
                  </label>
              ...
              <div className="flex items-center justify-between text-sm text-slate-400">
                <label className="inline-flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" aria-label="Remember me" className="w-4 h-4 rounded border-white/10 bg-transparent" />
              ...
              <div className="flex flex-col gap-2 items-center">
                <button
                  ...
                  className=" flex justify-center items-center w-full rounded-xl py-2 px-4 font-semibold bg-gradiant-to-r from-red-100 to bg-red-400 shadow-lg hover:translate-y-[-2px] transition-transform">
              ...
              <div className="text-center text-sm text-slate-400 mt-1">
                <p>Don't have an account? <a href="./register" className="text-sky-300 hover:underline">Register</a></p>
              </div>



              <div className="min-h-[1.25rem] mt-1" aria-live="polite">
                {shaking && <span className="inline-block text-sm px-2 py-1 rounded-md bg-rose-600/10 text-rose-200">Please provide a valid email and password</span>}
                {submitted && <span className="inline-block text-sm px-2 py-1 rounded-md bg-emerald-400/10 text-emerald-100">Logged in — welcome!</span>}
              </div>
            </form>
          </div>
        </section>
        ...
        <style jsx>{`
          ...
          /* small helper to mimic scale .99 */
